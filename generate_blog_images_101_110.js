...
const blogsToGenerate = blogPosts.slice(100, 110); // Blogs 101 to 110

const gradients = [
  { bg1: '#0F172A', bg2: '#16A34A', accent: '#4ADE80', theme: 'MANYCHAT ALTERNATIVE' },
  { bg1: '#1E1B4B', bg2: '#4F46E5', accent: '#818CF8', theme: 'REEL AUTOMATION' },
  { bg1: '#2E1065', bg2: '#9333EA', accent: '#C084FC', theme: 'LINKTREE VS DM' },
  { bg1: '#3F0713', bg2: '#E11D48', accent: '#FB7185', theme: 'SHADOWBAN SAFETY' },
  { bg1: '#064E3B', bg2: '#0D9488', accent: '#2DD4BF', theme: 'FREE AUTO-REPLY' },
  { bg1: '#172554', bg2: '#2563EB', accent: '#60A5FA', theme: 'KLAVIYO SYNC' },
  { bg1: '#4C1D95', bg2: '#7C3AED', accent: '#A78BFA', theme: '3-WAY COMPARISON' },
  { bg1: '#701A75', bg2: '#C026D3', accent: '#E879F9', theme: 'VIRAL TRIGGERS' },
  { bg1: '#431407', bg2: '#EA580C', accent: '#FB923C', theme: 'STORY MENTIONS' },
  { bg1: '#052E16', bg2: '#15803D', accent: '#86EFAC', theme: 'DM FUNNEL GUIDE' }
];
...
async function generateImages() {
  console.log('Generating 10 unique high-resolution blog cover images (1200x630)...');

  for (let i = 0; i < blogsToGenerate.length; i++) {
    const blog = blogsToGenerate[i];
    const style = gradients[i % gradients.length];
    const imageNum = 101 + i;
    const filename = `blog_${imageNum}.jpg`;
    const outputPath = path.join(publicDir, filename);

    const safeCategory = escapeSvg((blog.category || 'GUIDE').toUpperCase());
    ...
    const svgImage = `
      <svg width="1200" height="630" viewBox="0 0 1200 630" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(100, 120)">
          <rect width="230" height="38" rx="19" fill="${style.accent}" fill-opacity="0.2" stroke="${style.accent}" stroke-width="1.5" />
          <text x="115" y="24" text-anchor="middle" fill="${style.accent}" font-family="system-ui, -apple-system, sans-serif" font-size="13" font-weight="800" letter-spacing="1.5">${safeCategory}</text>
        </g>
        ...
        <g transform="translate(100, 490)">
          <text x="0" y="0" fill="${style.accent}" font-family="system-ui, -apple-system, sans-serif" font-size="14" font-weight="700" letter-spacing="1">2026 INSTAGRAM GROWTH GUIDE - ${escapeSvg((blog.readTime || '10 min read').toUpperCase())}</text>
        </g>
      </svg>
    `;
  ...
  console.log('\n🎉 Successfully generated all 10 blog cover images in public/!');
}
