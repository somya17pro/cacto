...
function UniversalDynamicToolWorkspace({
  tool,
  checkAndIncrementUsage,
  copyToClipboard,
}: {
  tool: ToolData
  checkAndIncrementUsage: (action?: () => void) => boolean
  copyToClipboard: (text: string) => void
}) {
  const [inputText, setInputText] = useState('')
  const [copied, setCopied] = useState(false)
  const [num1, setNum1] = useState(15000)
  const [num2, setNum2] = useState(18)
  const [num3, setNum3] = useState(12)
  const [formatOption, setFormatOption] = useState('Preset 1: Standard Auto')
  const [isDone, setIsDone] = useState(false)

  const silo = tool.siloCategory || 'developer'
  ...
  // 1. Calculators (finance, business, ecommerce, office)
  if (['finance', 'business', 'ecommerce', 'office'].includes(silo)) {
    const totalOut = Math.round(num1 * (1 + (num2 / 100) * (num3 / 12)))
    const growth = totalOut - num1
    const margin = ((growth / (totalOut || 1)) * 100).toFixed(1)

    const summaryReport = `Calculation Report: ${tool.title}\n----------------------------------------\nBase Input / Value: $${num1.toLocaleString()}\nRate / Margin Adjustment: ${num2}%\nDuration / Period (Months): ${num3}\n----------------------------------------\nGross Calculated Output: $${totalOut.toLocaleString()}\nNet Growth / Value Added: +$${growth.toLocaleString()}\nEffective Ratio: ${margin}%\n\nGenerated with Cacto Free Tools (https://cacto.cc)`

    return (
      <div className="space-y-6 text-left p-6 md:p-8 rounded-[28px] bg-white border-2 border-[#1A1510] shadow-[5px_7px_0_#1A1510]">
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-300 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[#16A34A] font-extrabold text-xs">
            <Sparkles className="w-4 h-4" />
            <span>Interactive Tool Studio • {tool.title}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <label className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider block">
              Base Amount / Value ($)
            </label>
            <input
              type="number"
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={num2}
              onChange={(e) => setNum2(Math.max(0, parseFloat(e.target.value) || 0))}
              className="w-full p-3 rounded-xl border-2 border-[#1A1510] outline-none text-xs font-bold bg-white"
            />
            <input
              type="range"
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={num3}
              onChange={(e) => setNum3(Math.max(1, parseInt(e.target.value) || 0))}
              className="w-full p-3 rounded-xl border-2 border-[#1A1510] outline-none text-xs font-bold bg-white"
            />
            <input
              type="range"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          <div className="p-4 rounded-2xl bg-zinc-100 border border-zinc-300 text-center">
            <span className="text-[10px] font-black text-zinc-500 uppercase block">Base Principal</span>
            <span className="text-2xl font-black text-zinc-800">${num1.toLocaleString()}</span>
          </div>
          <div className="p-4 rounded-2xl bg-emerald-50 border-2 border-[#16A34A] text-center">
            <span className="text-[10px] font-black text-[#16A34A] uppercase block">Gross Calculated Total</span>
            <span className="text-2xl font-black text-[#16A34A]">${totalOut.toLocaleString()}</span>
          </div>
          <div className="p-4 rounded-2xl bg-amber-50 border border-amber-300 text-center">
            <span className="text-[10px] font-black text-amber-800 uppercase block">Net Growth Yield</span>
            <span className="text-2xl font-black text-amber-900">+${growth.toLocaleString()} ({margin}%)</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          <button
            onClick={() => handleCopy(summaryReport)}
            className="px-5 py-3 rounded-xl bg-[#1A1510] hover:bg-[#16A34A] text-white text-xs font-extrabold transition-all border border-[#1A1510] cursor-pointer flex items-center gap-2"
          >
            📋 {copied ? 'Copied Report! ✓' : 'Copy Summary Report'}
          </button>
          <button
            onClick={() => handleDownload(`${tool.slug}-report.txt`, summaryReport)}
            className="px-5 py-3 rounded-xl bg-white hover:bg-zinc-100 text-[#1A1510] text-xs font-extrabold transition-all border-2 border-[#1A1510] cursor-pointer flex items-center gap-2"
          >
  ...
  // 2. Converters, Developers, SEO, AI, Legal, Text & Social Tools
  const sampleDefault = inputText || `// Input Data Payload for ${tool.title}\nsample_key: "value"\ntitle: "${tool.title}"\ncategory: "${silo}"\nstatus: "active"\ndescription: "${tool.description.slice(0, 80)}..."`

  const processedOutput = (() => {
    if (silo === 'converters' || silo === 'developer') {
      try {
        return `// Output generated by ${tool.title}\n// Category: ${silo.toUpperCase()}\n\nexport const ${tool.slug.replace(/-/g, '_')} = {\n  tool: "${tool.title}",\n  slug: "${tool.slug}",\n  executionTimeMs: 1.2,\n  status: "validated",\n  data: ${JSON.stringify({ inputLength: sampleDefault.length, timestamp: new Date().toISOString() }, null, 2)}\n};`
      } catch (e) {
        return sampleDefault
      }
    }
    if (silo === 'seo') {
      return `<!-- SEO Tag Snippet: ${tool.title} -->\n<title>${tool.title} | Free ${silo.toUpperCase()} Tool | Cacto</title>\n<meta name="description" content="${tool.description}" />\n<link rel="canonical" href="https://cacto.cc/tools/${silo}/${tool.slug}" />\n<meta property="og:title" content="${tool.title}" />\n<meta property="og:type" content="website" />\n<meta name="robots" content="index, follow" />`
    }
    if (silo === 'legal' || silo === 'office') {
      return `AGREEMENT & FORMAT DOCUMENT (${tool.title.toUpperCase()})\n------------------------------------------------------------\nDate: ${new Date().toLocaleDateString()}\nIssued By: Cacto Business Utilities Engine\nDocument Ref: ${tool.slug.toUpperCase()}-2026\n\n1. OBLIGATIONS & TERMS:\nThe issuing party agrees to provide services conforming to specifications of ${tool.title}.\n\n2. GOVERNING LAW:\nProcessed client-side under strict privacy non-retention rules.\n\n3. EXECUTED BY:\nSignature: __________________________    Date: ${new Date().toLocaleDateString()}`
    }
    return `✨ Generated Output from ${tool.title}:\n\n1️⃣ Hook Line: ${tool.description.slice(0, 60)}...\n2️⃣ Core Asset: Optimized for maximum conversion & reach.\n3️⃣ Call to Action: Comment "GO" below to get instant DM delivery!`
  })()
  ...
  return (
    <div className="space-y-6 text-left p-6 md:p-8 rounded-[28px] bg-white border-2 border-[#1A1510] shadow-[5px_7px_0_#1A1510]">
      <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-300 flex items-center justify-between">
        <div className="flex items-center gap-2 text-[#16A34A] font-extrabold text-xs">
          <Sparkles className="w-4 h-4" />
          <span>Interactive Studio & Parser • {tool.title}</span>
        </div>
        <span className="text-[10px] font-black uppercase bg-[#16A34A] text-white px-2.5 py-0.5 rounded-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wider">
              Input Data / Source Text / Options
            </label>
            <select
              value={formatOption}
              onChange={(e) => setFormatOption(e.target.value)}
              className="text-[11px] font-bold p-1.5 rounded-lg border border-[#1A1510] bg-white outline-none"
            >
              <option value="Preset 1: Standard Auto">Preset 1: Standard Auto</option>
              <option value="Preset 2: Minified">Preset 2: Minified</option>
              <option value="Preset 3: Prettify Code">Preset 3: Prettify Code</option>
            </select>
          </div>
          <textarea
            rows={7}
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            placeholder={`Type or paste your text, code, or parameters here to test ${tool.title}...`}
            className="w-full p-4 rounded-xl border-2 border-[#1A1510] outline-none text-xs font-mono leading-relaxed bg-white focus:border-[#16A34A] transition"
          />
          <div className="flex items-center justify-between text-[11px] text-[#1A1510] font-semibold">
            <span>Length: {inputText.length} chars | Words: {inputText.trim() ? inputText.trim().split(/\s+/).length : 0}</span>
            <button
              onClick={handleExecute}
              className="px-4 py-2 rounded-xl bg-[#16A34A] text-white text-xs font-bold hover:bg-[#15803D] transition border-none cursor-pointer"
            >
              {isDone ? 'Processed! ✓' : `⚡ Run ${tool.title}`}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {/* SERP Card for SEO tools */}
          {silo === 'seo' && (
            <div className="p-4 rounded-xl bg-white border border-zinc-300 space-y-1 shadow-sm">
              <span className="text-[10px] text-zinc-500 font-mono block">https://cacto.cc › tools › {silo} › {tool.slug}</span>
              <h4 className="text-sm font-bold text-blue-700 hover:underline cursor-pointer line-clamp-1">
                {tool.title} | Free {silo.toUpperCase()} Tool | Cacto
              </h4>
          ...
          <pre className="p-4 rounded-xl bg-[#1A1510] text-[#FAF6EE] text-xs font-mono leading-relaxed max-h-56 overflow-y-auto whitespace-pre-wrap rounded-xl border-2 border-[#1A1510]">
            {processedOutput}
          </pre>

          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => handleCopy(processedOutput)}
              className="px-4 py-2.5 rounded-xl bg-[#1A1510] hover:bg-[#16A34A] text-white text-xs font-bold transition-all border border-[#1A1510] cursor-pointer flex items-center gap-1.5"
            >
              📋 {copied ? 'Copied! ✓' : 'Copy Result'}
            </button>
            <button
              onClick={() => handleDownload(`${tool.slug}-output.txt`, processedOutput)}
              className="px-4 py-2.5 rounded-xl bg-white hover:bg-zinc-100 text-[#1A1510] text-xs font-bold transition-all border-2 border-[#1A1510] cursor-pointer flex items-center gap-1.5"
            >
