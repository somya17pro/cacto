# Data playbook — which ZenABM MCP tool to call for each section

Tool names below are shown unprefixed (e.g. `get_creative_performance`). At runtime they appear as
`mcp__<zenabm-server>__get_creative_performance` — match by the trailing name. Two windows matter everywhere:
...
— stop and help them connect (see `onboarding.md`). Never invent numbers.

## 1. Account scorecard (spend, ads, CPC, CPM, CTR, eff-CPC-to-LP)
- `get_linkedin_metrics` for cur and prev → impressions, clicks, engagements, cost. Derive CPC, CPM, CTR
  (`metrics.md`).
